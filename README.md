...
> **项目状态：0.1.0，早期开发阶段。** 项目仍在活跃开发中，请按早期版本对待：
>
> - **前端**：仍有较多已知 bug 未修复。
...
应用涉及两类本地数据：你管理的**文档库**，以及由它派生的数据库与向量。
...
### 派生存储（`%USERPROFILE%\.iftree\`）

导入后解析出的结构、向量与附件写入用户数据目录（可用 `IFTREE_HOME` 覆盖）：
...
│   │   ├── importers.mjs # 配合 import-formats/ 解析 chm/txt/md/pdf/docx
