...
test('applyMerge 写回后由写分发收尾增量同步派生索引：BM25 增量、受影响集合不进响应、向量零耦合（4-6-1）', async () => {
  await withStore(async (store) => {
    ...
    assert.equal(applied.applied, true);
    // 落主干后收尾对该文档做一次「增量」派生索引同步：BM25 用事务算出的 touched/deleted 增量集合
    // （不再整篇重建，O(改动) 而非 O(全文档)），向量零耦合、保存不 embed。这里合并的是分支对 a 的
    // 内容改动 → touched 恰为 a、无删除。
    assert.equal(maintained.length, 1, '落主干后对该文档维护一次');
    assert.equal(String(maintained[0][0]), String(doc.id));
    assert.deepEqual(maintained[0][1].deletedNodeIds.map(String), [], '纯内容合并无删除');
    assert.deepEqual(maintained[0][1].touchedNodeIds.map(String), [String(a.id)], 'BM25 增量同步只触达被合并的 a');
    assert.ok(
      !('touchedNodeIds' in applied) && !('deletedNodeIds' in applied) && !('vectorStaleNodeIds' in applied),
      '受影响节点集（维护用增量 hint）不进响应'
    );
  });
});
