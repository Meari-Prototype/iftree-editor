...
test('无锚 ctx 拒绝建卷并回滚（15-10-4 非导航文档必有库内实体锚）', async () => {
  await withStore(async (store) => {
    // 缺建锚能力（ctx 无 writeMemoryAnchor）：建卷前即拒。
    await assert.rejects(runDatabaseWrite(store, deliverPayload(), {}), /无锚卷|15-10-4/);
    // 建锚抛错：已建的卷回滚删除（未锚卷无 source 行、deleteDoc 放行），不留残卷。
    await assert.rejects(
      runDatabaseWrite(store, deliverPayload(), { writeMemoryAnchor() { throw new Error('symlink 失败'); } }),
      /建锚失败|回滚/
    );
    // 两次拒绝后库内不得残留任何记忆卷（空库自带 1 个导航虚拟文档）。
    assert.equal(store.db.prepare("SELECT COUNT(*) AS c FROM docs WHERE meta LIKE '%memoryVolume%'").get().c, 0);
    assert.equal(store.db.prepare('SELECT COUNT(*) AS c FROM docs').get().c, 1);
  });
});
