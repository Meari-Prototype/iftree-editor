...
} from '../dist/src/backend/memory/volumes.js';
...
test('volumeNodesFromTurnMessages：消息粒度局部展开、工具事件为子节点、一律不受控', async () => {
  const { volumeNodesFromTurnMessages } = await import('../dist/src/backend/llm/agent-runtime.js');
...
test('messagesFromClaudeTranscript：只取 user 原话 + assistant 文本/工具，过滤系统合成行与坏行', async () => {
  const { messagesFromClaudeTranscript } = await import('../dist/src/core/session-transcript.js');
  const lines = [
...
test('isLegalEventVolumeLayout：. / .. / 含分隔符段判非法（堵锚目录逃逸），正常单段合法', async () => {
  const { isLegalEventVolumeLayout } = await import('../dist/src/backend/memory/anchor-layout.js');
  // 正常租户 + 工作区：合法。
