...
test('messagesFromClaudeTranscript：只取 user 原话 + assistant 文本/工具，过滤系统行与坏行', async () => {
  ...
  // 只剩 1 条用户原话 + 1 条 assistant；queue-operation/attachment/纯 tool_result/坏行/空行全过滤。
  assert.equal(messages.length, 2);
  assert.equal(messages[0].role, 'user');
  assert.equal(messages[0].content, '帮我审一下代码');
  assert.equal(messages[1].role, 'assistant');
  assert.equal(messages[1].content, '我来读文件'); // thinking 不进卷
  assert.equal(messages[1].toolEvents.length, 1);
  assert.match(messages[1].toolEvents[0].name, /Read/);
});
