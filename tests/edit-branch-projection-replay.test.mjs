...
test('node.insert：stage 拒绝 trust_level（受控只走 human certify，18-3）', async () => {
  await withStore(async (store) => {
    const { doc } = setupBranch(store, 'InsertTrustLevel', ['a']);
    const branch = store.activeEditBranchForBaseDoc(doc.id, 'human');
    // 分支编辑不能凭空铸造受控状态：trust_level 在 stage 入口即被拒（even owner=human），
    // 标受控的唯一合法入口是 human certify（18-3，trust ∈ content_hash / A5-2）。
    assert.throws(
      () => store.stageEditBranchNodeInsert(branch, {
      ...
      /trust_level|certify/,
      'stage 应拒绝 trust_level'
    );

    // 去掉 trust_level 的同一插入照常落主干、其余字段不受影响，重放后 trust 为空（不受控）。
    store.stageEditBranchNodeInsert(branch, {
      parentId: doc.rootNodeId,
      text: '记忆条目正文',
      nodeTitle: '[反馈] 示例',
    ...
    const saved = store.db.prepare(
      'SELECT node_note, trust_level FROM nodes WHERE doc_id = ? AND node_title = ?'
    ).get(doc.id, '[反馈] 示例');
    assert.ok(saved, '插入节点已落主干');
    assert.equal(saved.node_note, '源 example.md');
    assert.ok(!saved.trust_level, '未经 certify 的分支插入节点 trust 为空');
  });
});

function lastBranchEntry(branch) {
  const diff = JSON.parse(branch.diff || '{}');
  const entries = Array.isArray(diff.entries) ? diff.entries : [];
  return entries[entries.length - 1] || null;
}

test('stage node.moveBefore：经 stage 方法入 diff，投影与重放一致', async () => {
  await withStore(async (store) => {
    const { doc, children, branch } = setupBranch(store, 'StageMoveBefore', ['a', 'b', 'c']);
    const { branch: fresh } = store.stageEditBranchNodeMoveBefore(branch, {
      nodeId: children.c.id,
      targetNodeId: children.a.id
    });
...
test('stage node.mergePrevious："前一兄弟"按投影态物化为 target_ref，而非主干态', async () => {
  await withStore(async (store) => {
    const { doc, children, branch } = setupBranch(store, 'StageMergePrevious', ['a', 'b', 'c']);
    // 分支内先把 c 移到 b 之前：投影序变为 a, c, b——此时 b 的前一兄弟是 c（主干态是 a）。
    const { branch: moved } = store.stageEditBranchNodeMoveBefore(branch, {
      nodeId: children.c.id,
      targetNodeId: children.b.id
    });
    const { branch: fresh } = store.stageEditBranchNodeMergePrevious(moved, {
      nodeId: children.b.id
    });

    const entry = lastBranchEntry(fresh);
    assert.equal(entry.kind, 'node.mergePrevious');
    assert.equal(String(entry.target_ref), String(children.c.id), 'target_ref 物化为投影态的前一兄弟 c');

    const projected = projectedChildTexts(store, fresh, doc.rootNodeId);
    assert.deepEqual(projected, ['a', 'c\n\nb'], '投影：b 并入 c');

    store.saveEditBranch({ baseDocId: doc.id, owner: 'human' });
    assert.deepEqual(savedChildTexts(store, doc.rootNodeId), projected, '保存所得 == 编辑所见（无重放时点漂移）');
  });
});

test('stage node.mergePrevious：首位节点无前一兄弟 → 空操作不产 entry', async () => {
  await withStore(async (store) => {
    const { children, branch } = setupBranch(store, 'StageMergePreviousNoop', ['a', 'b']);
    const { branch: fresh, changed } = store.stageEditBranchNodeMergePrevious(branch, {
      nodeId: children.a.id
    });
    assert.equal(changed, false, '无前一兄弟：no-op');
    assert.equal(lastBranchEntry(fresh), null, '不产生 entry');
  });
});

test('runDatabaseWrite 分支路由：node.moveBefore / node.mergePrevious 可经写入口 stage（原抛 not implemented）', async () => {
  await withStore(async (store) => {
    const { doc, children } = setupBranch(store, 'DispatchStage', ['a', 'b', 'c']);

    await runDatabaseWrite(store, {
      action: 'node.moveBefore',
      docId: doc.id,
      nodeId: children.c.id,
      targetNodeId: children.a.id
    });
    await runDatabaseWrite(store, {
      action: 'node.mergePrevious',
      docId: doc.id,
      nodeId: children.a.id
    });

    const active = store.activeEditBranchForBaseDoc(doc.id, 'human');
    const projected = projectedChildTexts(store, active, doc.rootNodeId);
    assert.deepEqual(projected, ['c\n\na', 'b'], '投影：c 前移后 a 并入 c');
