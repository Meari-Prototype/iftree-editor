...
4. **切得细。** 粒度在导入期锁定，入库后没有段落切分/合并的通道。
   切到「章节层 → 段落容器 → 句子」：句子按句末标点（。！？!?.）切，
   一句一个节点；段落作为容器节点（首句可作为容器正文，或容器留空做虚拟节点）。
  ...
  "nodes": [
    {
      ...
      "trustLevel": "不受控",
      "children": [
        { "address": "1-1-1", "text": "本法所称电子签名……。", "trustLevel": "不受控" }
      ]
    },
    {
      "address": "1-2",
      ...
      "sourcePosition": 3.5,
      "trustLevel": "不受控",
      "children": [ { "address": "1-2-1", "text": "……", "trustLevel": "不受控" } ]
    }
  ]
}
```
...
- `address`：必填。根节点是 `1`（系统建），顶层节点从 `1-1` 起；
  每个父节点下子地址末位从 1 连续递增，不跳号。树的嵌套用 `children`。
...
2. **写脚本**：在 LLM 工作区（`.iftree-llm-workspace`）写一个一次性 node 脚本，
   ...
   按行/按模式定位结构边界；句子用句末标点切；正文一律 `slice` 原文，不要重新拼写。
3. **校验**：
   ```
   db import-json <工作区>/tree.json <源文件路径> --dry-run
   ```
   读返回的 JSON 报告，按 `errors` 与 `gaps` 修脚本重跑，直到 `ok: true`：
   - `missing`——正文在源文中不存在：九成是脚本改动了内部空白/换行，
     或源文有不可见字符；对照 `textPreview` 定位。
   - `out_of_order`——正文存在但位置在已消费区间之前：JSON 顺序与源文不一致，
     检查脚本的遍历顺序。
   - `address_*`——地址不连续/前缀错：检查编号生成逻辑。
   - `virtual_source_position`——虚拟容器缺半步句位。
   - `gaps`——源文有未被任何节点覆盖的区间：看 `preview` 决定补节点
     还是确认无需入库（页眉、页码、装饰线）。gap 默认拒绝导入，
     全部确认合理后加 `--allow-gaps` 放行。
4. **导入**：去掉 `--dry-run` 正式入库（需要向量时加 `--vectors`）。
...
- [ ] 前序顺序 = 源文顺序
- [ ] 地址连续、父前缀正确、顶层从 `1-1` 起
- [ ] 虚拟容器：`text` 空、`nodeTitle` 有名、`sourcePosition` 半步
- [ ] 所有节点 `trustLevel: "不受控"`
- [ ] 粒度到句子级
- [ ] `--dry-run` 报告 `ok: true`，gap 全部可解释
