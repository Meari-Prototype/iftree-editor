...
3. **校验**：
   ...
   ```
   读返回的 JSON 报告，只需按 `missing` / `out_of_order` 两类错误修脚本重跑，直到 `ok: true`：
   - `missing`——正文在源文中不存在：九成是脚本改动了内部空白/换行，
     或源文有不可见字符；对照 `textPreview` 定位。
   - `out_of_order`——正文存在但位置在已消费区间之前：JSON 顺序与源文不一致，
     检查脚本的遍历顺序。
   地址、句子层、gap 都由系统处理、你不用管：地址按 children 前序自动生成；段落正文由系统按句末
   标点切成句子（段落本身变成空容器、句子作它的子）；源文里你没切到的区间——段落间空行、分页符靠
   段落空容器的位置表达边界、不补节点，装饰线（⸻）和漏掉的正文段则补成正文节点，都不丢。
...
- [ ] `--dry-run` 报告 `ok: true`（gap 由系统自动补，不用你解释）
