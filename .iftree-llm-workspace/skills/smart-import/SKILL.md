...
4. **导入**：去掉 `--dry-run` 正式入库（需要向量时加 `--embed`）。
   命令自动完成：源路径查重 → 建文档 → 批量建树（期间临时切增量编辑模式，结束切回完整模式）
   → 绑定源文档层与句位对照（导入后的文档支持选区高亮与句位回溯）。写入中途失败会删除已建的文档（删除也失败时，残留文档须手动删掉才能重导）。
   同一份源文已经导入过时，正式导入会报重复（`--dry-run` 在查重之前返回，不会提示）；要重导须先删除旧文档。
5. **留档**：脚本与 `tree.json` 留在工作区，默认保留 30 天——
   它们是这次导入的证据链，可追溯、可重跑、可修正后重导（重导前先删除旧文档）。
