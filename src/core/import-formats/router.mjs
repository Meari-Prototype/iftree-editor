...
import { readChmSourceDocument, readPdfSourceDocument, readSourceDocument } from '../importers.mjs';
import { normalizeSourceMarkdown } from '../source-doc.mjs';
import { importChmDocument } from './chm.mjs';
import { importPdfDocument } from './pdf.mjs';
import { importPythonDocument } from './python.mjs';
...
async function importDirectDocument(filePath, extension) {
  ...
  if (extension === '.md' || extension === '.txt') {
    ...
    text = normalizeSourceMarkdown(readFileSync(filePath, 'utf8'));
...
export async function importRecordsForFile(filePath, options = {}) {
  ...
  if (extension === '.py') return importPythonDocument(filePath, options);
  ...
  if (extension === '.xlsx' || extension === '.csv') {
    throw new Error(`${extension} 只支持向量导入入口，不支持普通文档导入。`);
  }
