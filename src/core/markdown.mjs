...
export function markdownToPlainText(markdown, options = {}) {
  const lines = [];
  for (const block of parseMarkdownBlocks(markdown)) {
...
export function markdownBlockToPlainText(block, options = {}) {
  if (!block) return '';
...
export function markdownInlineToPlainText(tokens = [], options = {}) {
...
const TEX_SYMBOLS = Object.freeze({
  alpha: 'α',
  beta: 'β',
  gamma: 'γ',
  delta: 'δ',
...
const SUBSCRIPT_CHARS = Object.freeze({
  '0': '₀',
  '1': '₁',
  '2': '₂',
  '3': '₃',
...
const SUPERSCRIPT_CHARS = Object.freeze({
  '0': '⁰',
  '1': '¹',
  '2': '²',
  '3': '³',
...
export function renderTexMathToText(tex) {
  return normalizeMathSpacing(renderTexExpression(String(tex || '').trim(), 0).text);
}

function renderTexExpression(source, startIndex = 0, stopChar = '') {
  let text = '';
...
function readTexScript(source, index) {
  if (source[index] === '{') {
    const group = renderTexExpression(source, index + 1, '}');
...
function renderScriptText(value, table, marker) {
