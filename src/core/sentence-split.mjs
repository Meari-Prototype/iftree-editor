...
export function splitSentenceSpans(text, options = {}) {
...
export function isBlockMath(text) {
  const trimmed = String(text ?? '').trim();
  return trimmed.startsWith('$$') || trimmed.startsWith('\\[');
}

// 若 index 处是块级公式开定界符，返回公式块结束位置（含闭定界符）；否则返回 index（无进展）。
function readBlockMathEnd(text, index, end) {
...
function isSentenceEnding(text, index, bracketDepth, parenDepth, endings) {
...
function isInternalAsciiDot(text, index) {
  const previous = text[index - 1] || '';
  const next = text[index + 1] || '';
  return /[A-Za-z0-9]/.test(previous) && /[A-Za-z0-9]/.test(next);
}

function consumeClosingPunctuation(text, start, end) {
  let index = start;
  while (index < end && CLOSING_PUNCT.test(text[index])) index += 1;
  return index;
}

function isIgnorableSpanText(text) {
  const trimmed = String(text || '').trim();
