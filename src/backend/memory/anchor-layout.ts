...
export function isPlaceholderTenant(tenant: unknown): boolean {
  const value = String(tenant || '').trim();
  return value === '' || value === PLACEHOLDER_TENANT;
}

export function isPlaceholderWorkspace(workspace: unknown): boolean {
...
function isUnsafeSegment(value: unknown): boolean {
  const v = String(value || '').trim();
  return v === '.' || v === '..' || v.includes('/') || v.includes('\\');
}

// 事件卷锚是否落在合法的 <租户>/<工作区>/ 结构下（两层都非占位、且都是不逃逸的单段普通名）。
export function isLegalEventVolumeLayout(tenant: unknown, workspace: unknown): boolean {
...
export function eventVolumeAnchorDir(libraryRoot: string, tenant: string, workspace: string): string {
  return join(libraryRoot, EVENT_VOLUME_ROOT, tenant, workspace);
}

// 长期核心记忆目录：<libraryRoot>/memory/<租户>/[<工作区>/]（无工作区=通用核心记忆，有=项目核心记忆）。
export function longTermMemoryDir(libraryRoot: string, tenant: string, workspace = ''): string {
...
interface IllegalEventVolumeMessageOptions {
  tenant?: string;
  workspace?: string;
  linkPath?: string;
}

export function illegalEventVolumeMessage({ tenant, workspace, linkPath }: IllegalEventVolumeMessageOptions = {}): string {
