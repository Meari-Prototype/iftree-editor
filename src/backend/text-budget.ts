...
export function clip(text, max = CHAR_LIMITS.line) {
...
export function clipText(text, limit = CHAR_LIMITS.fullText, { label = '全文' } = {}) {
  const s = String(text ?? '');
...
export function clipRows(rows, max = CHAR_LIMITS.rows) {
