...
import { requireStableId } from '../db/ids.mjs';
import { computeSnapshotDiff } from '../db/snapshot-history.mjs';
import { gcObjects } from '../db/object-store.mjs';
...
function commitAncestry(store, docId) {
...
export function listHistory(store, docId) {
...
export function nodeHistory(store, docId, address, { scope = 'subtree' } = {}) {
...
export function gcHistoryObjects(store) {
  return store.withTransaction(() => gcObjects(store.db));
}

export function saveHistorySnapshot(store, { docId, summary = '保存版本', owner = 'human' }) {
...
export function restoreCommit(store, commitId) {
  return store.withTransaction(() => {
    const commit = store.db.prepare('SELECT * FROM commits WHERE id = ?').get(commitId);
 ...
 */
export function revertCommit(store, { commitId, owner = 'human', summary = null } = {}) {
  const normalizedCommitId = requireStableId(commitId, 'revert commitId');
  return store.withTransaction(() => {
    const target = store.db.prepare('SELECT * FROM commits WHERE id = ?').get(normalizedCommitId);
    if (!target) throw new Error(`revert 找不到 commit ${commitId}`);
    if (!target.parent_commit_id) throw new Error('revert 不能撤销初始提交（无父提交）');
    const parentRow = store.db.prepare('SELECT * FROM commits WHERE id = ?').get(target.parent_commit_id);
    ...
    if (head?.head_commit_id) {
      const prev = store.commitSnapshot(head.head_commit_id);
      if (prev?.nodes) entries.push(...store.computeDiff(prev, finalSnapshot));
    }
