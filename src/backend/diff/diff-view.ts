...
export function buildAxiomDiffRows(baseAxioms: DiffRow[] = [], projectedAxioms: DiffRow[] = []) {
  ...
  const rows: DiffRow[] = [];
  const stats: Record<string, number> & { added: number; deleted: number; modified: number } = {
    added: 0,
    deleted: 0,
    modified: 0
  };
...
// —— 三方合并预览（editBranch.threeWayMerge）的结构化折叠 ——
// 预览结果含逐节点 resolution（绝大多数是 unchanged）+ 扁平 conflicts；裸结果把全部节点列出来，
// 文档一大就不可用。这里折叠 unchanged、只留有裁决/冲突的节点，给计数与冲突清单。
// 返回仍是结构化数据（query-api 直接回给调用方）；文本渲染在 merge-text（shell 侧消费）。

export interface MergePreviewNode {
  id?: unknown;
  address?: unknown;
  title?: unknown;
  resolution?: string;
...
export function summarizeThreeWayMerge(res: MergePreviewResult | null | undefined = {}, { maxNodes = 200 } = {}) {
  ...
  return {
    ...
    nodes: changed,
    ...(branch ? { branch } : {}),
    // 提示不指向 detail=raw——那是内部口径、MCP/CLI 都没有该入口；要全量逐节点明细走 diff（json=true）。
    summary: 'merge 预览摘要：未改节点已折叠省略，只列净变化与冲突；要全量逐节点明细用 diff（json=true）'
  };
}
