...
import { buildTreeIndex, getDescendants, patchNode, removeNode, toTreeNode } from '../../core/node-model.js';
...
export function createSession(docId) {
  return {
...
function upsertNodes(index, rows) {
  const nodes = [];
...
function reorderChildren(index, parentId) {
...
export function ingestRoot(state, rootRow) {
...
export function ingestChildren(state, patch = {}) {
...
export function reconcileChildren(state, patch = {}) {
  const parentId = normalizeId(patch.parentId);
  if (parentId == null) return state;
  const index = state.index;
  const rows = Array.isArray(patch.rows) ? patch.rows : [];
...
function loadedChildCount(state, nodeId) {
...
export function makeChildCountOf(state) {
  return (address) => {
...
function fetchBoundaryOf(state, nodeId) {
...
export function planHotFetches(state, options = {}) {
...
function collectFetches(state, nodeIds) {
  const fetches = [];
  const seen = new Set();
...
export function nextBackgroundFetch(state) {
...
export function isFullyLoaded(state) {
...
export function maxDepthOf(state) {
  let max = 1;
...
function commitView(state, patch) {
...
function collapsedForDepth(state, collapsed, limit) {
  const next = new Set();
...
function promoteIfLayerExpanded(state, { depthLimit, collapsed, expanded }) {
...
export function setMultiSelected(state, ids) {
  return commitView(state, { multiSelected: idSet(ids) });
}

export function setDepthLimit(state, value) {
...
export function applyViewState(state, raw = {}) {
  const cap = maxDepthOf(state);
...
export function viewStatePayload(state) {
  const list = (s) => [...(s || new Set())].map(normalizeId).filter(Boolean);
...
export function toggleCollapsed(state, nodeId, options = {}) {
  const id = normalizeId(nodeId);
  ...
  let result = { depthLimit: view.depthLimit, collapsed: nextCollapsed, expanded: nextExpanded };
...
export function expandOneLevel(state, nodeId, options = {}) {
  const id = normalizeId(nodeId);
  ...
  const result = { depthLimit: nextDepthLimit, collapsed: nextCollapsed, expanded: nextExpanded };
...
function viewPatchUnchanged(view, patch) {
...
export function applyViewSnapshot(state, snapshot = {}) {
  const patch = {};
...
export function projectToLegacyDoc(state) {
  const root = state.index.root;
  if (!root) return { tree: null, idByAddress: {}, depthStats: { maxDepth: 1, depths: [1] } };

  const cloneNode = (node) => ({ ...node, children: [] });
